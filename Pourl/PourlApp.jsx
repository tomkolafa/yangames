// POURL — App root: full-viewport mobile shell, nav, screen routing

function PourlBottomNav({ screen, onTab, theme }) {
...
function PourlApp() {
  ...
  var [theme,   setTheme]   = React.useState(function () { return localStorage.getItem('pourl-theme') || 'clean-light'; });
  var [charId,  setCharId]  = React.useState(function () { return localStorage.getItem('pourl-char')  || 'B'; });
  ...
  var saveTheme = function (t) { setTheme(t); localStorage.setItem('pourl-theme', t); };
  var saveChar  = function (c) { setCharId(c); localStorage.setItem('pourl-char', c); };
  ...
  return (
    <div className="app-root" data-theme={theme}>
      <div className="phone-shell">
        {screen === 'home' && React.createElement(window.PourlHomeScreen, {
          theme, charId, setCharId: saveChar, onPlay: startGame,
        })}
        {screen === 'game' && React.createElement(window.PourlGameScreen, {
          key: gameKey,
          theme, charId,
          onBack:  function () { setScreen('home'); },
          onBoard: function () { setScreen('leaderboard'); },
        })}
        {screen === 'leaderboard' && React.createElement(window.PourlLeaderboardScreen, {
          theme, charId,
        })}
        {screen === 'settings' && React.createElement(window.PourlSettingsScreen, {
          theme, setTheme: saveTheme,
        })}
        {showNav && React.createElement(PourlBottomNav, { screen, onTab: handleTab, theme })}
