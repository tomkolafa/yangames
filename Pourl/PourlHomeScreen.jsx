// POURL — Home Screen: pet (mascot) selector, play, personal best

// Mini canvas preview — front-facing pet portrait (reuses PourlGame.drawFace)
function PourlPreviewCanvas({ charId, size }) {
  ...
  React.useEffect(function () {
    ...
    try { window.PourlGame.drawFace(ctx, charId, s * 0.78); } catch (e) { console.error(e); }
    ctx.restore();
  }, [charId, s]);
  ...
  return (
    <canvas
      ref={canvasRef}
      style={{ display: 'block', pointerEvents: 'none', width: s + 'px', height: s + 'px' }}
    />
  );
}

function PourlHomeScreen({ theme, charId, setCharId, onPlay }) {
  ...
  var LOGO_COLORS  = ['#4FA3E0', '#56C98A', '#F4B942', '#E8755C', '#9B6BD6'];
  var LOGO_LETTERS = ['P', 'O', 'U', 'R', 'L'];
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ fontSize: 12, fontWeight: 600, color: mutedColor }}>
          Pour & sort — one colour per tube
        </div>
      </div>

      {/* Pet (mascot) selector */}
      <div style={{ width: '100%' }}>
        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: mutedColor, marginBottom: 8, marginTop: 12 }}>
          Choose Your Buddy
        </div>
          ...
          {Game.CHARACTERS.map(function (char) {
            ...
            return (
              <button key={char.id} onClick={function () { setCharId(char.id); }} style={{
                ...
                <PourlPreviewCanvas charId={char.id} size={86} />
                <div style={{
      ...
      <div style={{
        ...
        {[
          { val: globalHi > 0 ? String(globalHi) : '—', label: 'Best Run' },
...
window.PourlHomeScreen = PourlHomeScreen;
