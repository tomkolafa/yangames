// POURL — Leaderboard Screen

// Front-facing pet portrait (reuses PourlGame.drawFace)
function PourlFacePortrait({ charId, size }) {
  ...
  return (
    <canvas ref={canvasRef} style={{ display: 'block', width: s + 'px', height: s + 'px', margin: '0 auto' }} />
  );
}

function PourlLeaderboardScreen({ theme, charId }) {
  ...
  React.useEffect(function () {
    if (window.YanLeaderboard && window.YanLeaderboard.ENABLED) {
      window.YanLeaderboard.fetchTop('pourl', { limit: 25 }).then(function (rows) {
        setOnline(rows || []);
      });
    } else {
      setOnline([]);
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--game-bg)', color: 'var(--game-text)', minHeight: 0 }}>
      ...
      <div style={{
        ...
        <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 22, letterSpacing: '0.05em' }}>
          Leaderboard
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px 28px', display: 'flex', flexDirection: 'column', gap: 20, minHeight: 0 }}>
        ...
        {/* ── Per-character bests ── */}
        <div>
          <SectionLabel>Your Best Run Per Buddy</SectionLabel>
              ...
              return (
                <div key={char.id} style={{
                  ...
                  <div style={{ marginBottom: 4 }}>
                    <PourlFacePortrait charId={char.id} size={44} />
                  </div>
        ...
        <div style={{ background: cardBg, border: '1px solid ' + cardBorder, borderRadius: 14, padding: '14px 12px' }}>
          ...
          <div style={{ display: 'flex' }}>
            {[
              { val: stats.games  || 0,   label: 'Runs'           },
              { val: stats.levels || 0,   label: 'Levels Solved'  },
              { val: globalHi || '—',     label: 'Best Run'       },
            ].map(function (item, i) {
              return (
          ...
          {loading ? (
              ...
              {fullLb.map(function (entry, i) {
                ...
                return (
                  <div key={i} style={{
                    ...
                    background: isYou ? (dark ? 'rgba(143,211,182,.17)' : 'var(--brand-tint)') : 'transparent',
                  }}>
                    ...
                    <div style={{
                      ...
                      <PourlFacePortrait charId={entry.char} size={36} />
                    </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: 12, color: mutedColor }}>
                Play a run to claim the top spot!
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{
          ...
          {[
            ...
            '🎨 You can only pour onto an empty tube or a matching colour',
            '✅ Finish a tube (one colour, full) to keep clearing levels',
            '↩️ 3 undos per level — get stuck with none left and the run ends',
            '📈 Harder levels are worth more; clear with no undo for a bonus',
          ].map(function (tip) {
            return (
...
window.PourlLeaderboardScreen = PourlLeaderboardScreen;
