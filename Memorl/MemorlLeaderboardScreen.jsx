// MEMORL — Leaderboard Screen

// Front-facing pet portrait (reuses MemorlGame.drawFace)
function MemorlFacePortrait({ charId, size }) {
  ...
  React.useEffect(function () {
    ...
    try { window.MemorlGame.drawFace(ctx, charId, s); } catch (e) { console.error(e); }
    ctx.restore();
  }, [charId, s]);
...
function MemorlLeaderboardScreen({ theme, charId }) {
  ...
  React.useEffect(function () {
    if (window.YanLeaderboard && window.YanLeaderboard.ENABLED) {
      window.YanLeaderboard.fetchTop('memorl', { limit: 25 }).then(function (rows) {
        setOnline(rows || []);
      });
    } else {
      setOnline([]);
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px 28px', display: 'flex', flexDirection: 'column', gap: 20, minHeight: 0 }}>
              ...
              return (
                <div key={char.id} style={{
                  ...
                  <div style={{ marginBottom: 4 }}>
                    <MemorlFacePortrait charId={char.id} size={44} />
                  </div>
              ...
              {fullLb.map(function (entry, i) {
                ...
                return (
                  <div key={i} style={{
                    ...
                    background: isYou ? (dark ? 'rgba(199,182,232,.17)' : 'var(--brand-tint)') : 'transparent',
                  }}>
                    ...
                    <div style={{
                      ...
                      <MemorlFacePortrait charId={entry.char} size={36} />
                    </div>
        ...
        <div style={{
          ...
          {[
            ...
            '👆 Tap them all back once they hide',
            '📈 Clear a round to grow the grid',
            '❤️ Three wrong taps and you\'re out',
          ].map(function (tip) {
            return (
...
window.MemorlLeaderboardScreen = MemorlLeaderboardScreen;
