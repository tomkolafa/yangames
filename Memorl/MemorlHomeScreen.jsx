// MEMORL — Home Screen with animated pet selector

// Mini canvas preview — draws a pet (side view) running in a card
function MemorlPreviewCanvas({ charId, size }) {
  ...
  React.useEffect(function () {
    ...
    function drawFrame(f) {
      ...
      try { window.MemorlGame.drawCharacter(ctx, charId, f, false); } catch (e) { console.error(e); }
      ctx.restore();
    }
...
function MemorlHomeScreen({ theme, charId, setCharId, onPlay }) {
  ...
  var LOGO_COLORS  = ['#C7B6E8', '#8FD3B6', '#F4B942', '#E8755C', '#79D0E0', '#F3A9C8'];
  var LOGO_LETTERS = ['M', 'E', 'M', 'O', 'R', 'L'];
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ fontSize: 12, fontWeight: 600, color: mutedColor }}>Memorize the pattern, tap it back</div>
      </div>
          ...
          {Game.CHARACTERS.map(function (char) {
            ...
            return (
              <button key={char.id} onClick={function () { setCharId(char.id); }} style={{
                ...
                <MemorlPreviewCanvas charId={char.id} size={86} />
                <div style={{
...
window.MemorlHomeScreen = MemorlHomeScreen;
