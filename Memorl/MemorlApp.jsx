// MEMORL — App root: full-viewport mobile shell, nav, screen routing

function MemorlBottomNav({ screen, onTab, theme }) {
...
function MemorlApp() {
  ...
  var [theme,   setTheme]   = React.useState(function () { return localStorage.getItem('memorl-theme') || 'clean-light'; });
  var [charId,  setCharId]  = React.useState(function () { return localStorage.getItem('memorl-char')  || 'B'; });
  ...
  var saveTheme = function (t) { setTheme(t); localStorage.setItem('memorl-theme', t); };
  var saveChar  = function (c) { setCharId(c); localStorage.setItem('memorl-char', c); };
  ...
  return (
    <div className="app-root" data-theme={theme}>
      <div className="phone-shell">
        {screen === 'home' && React.createElement(window.MemorlHomeScreen, {
          theme, charId, setCharId: saveChar, onPlay: startGame,
        })}
        {screen === 'game' && React.createElement(window.MemorlGameScreen, {
          key: gameKey,
          theme, charId,
          onBack:  function () { setScreen('home'); },
          onBoard: function () { setScreen('leaderboard'); },
        })}
        {screen === 'leaderboard' && React.createElement(window.MemorlLeaderboardScreen, {
          theme, charId,
        })}
        {screen === 'settings' && React.createElement(window.MemorlSettingsScreen, {
          theme, setTheme: saveTheme,
        })}
        {showNav && React.createElement(MemorlBottomNav, { screen, onTab: handleTab, theme })}
