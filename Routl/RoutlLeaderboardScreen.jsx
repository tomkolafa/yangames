// ROUTL — Leaderboard Screen

function RoutlFacePortrait({ charId, size }) {
  ...
  React.useEffect(function () {
    ...
    try { window.RoutlGame.drawFace(ctx, charId, s); } catch (e) { console.error(e); }
    ctx.restore();
  }, [charId, s]);
  return <canvas ref={canvasRef} style={{ display: 'block', width: s + 'px', height: s + 'px', margin: '0 auto' }} />;
}

function RoutlLeaderboardScreen({ theme, charId }) {
  ...
  React.useEffect(function () {
    if (window.YanLeaderboard && window.YanLeaderboard.ENABLED) {
      window.YanLeaderboard.fetchTop('routl', { limit: 25 }).then(function (rows) { setOnline(rows || []); });
    } else { setOnline([]); }
  }, []);
  ...
  if (online && online.length) {
    fullLb = online.slice(0, 10).map(function (r) {
      return { name: r.name, emoji: r.emoji, char: r.char_id || 'B', score: r.score, isYou: !!myName && r.name === myName };
    });
  } else {
    fullLb = [];
    Game.CHARACTERS.forEach(function (c) { if (hs[c.id]) fullLb.push({ name: myName || 'You', emoji: CHAR_EMOJI[c.id], char: c.id, score: hs[c.id], isYou: true }); });
  ...
  function SectionLabel({ children }) { return <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: mutedColor, marginBottom: 8 }}>{children}</div>; }
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--game-bg)', color: 'var(--game-text)', minHeight: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px 28px', display: 'flex', flexDirection: 'column', gap: 20, minHeight: 0 }}>
        <div>
          <SectionLabel>Your Best Per Captain</SectionLabel>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {Game.CHARACTERS.map(function (char) {
              var active = char.id === charId;
              var cc     = CHAR_COLOR[char.id];
              return (
                <div key={char.id} style={{ background: active ? (dark ? 'rgba(255,255,255,.09)' : 'var(--brand-tint)') : cardBg, border: '1px solid ' + (active ? cc : cardBorder), borderRadius: 12, padding: '12px 10px', textAlign: 'center' }}>
                  <div style={{ marginBottom: 4 }}><RoutlFacePortrait charId={char.id} size={44} /></div>
                  <div style={{ fontSize: 9, fontWeight: 700, color: cc, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{char.name}</div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--game-text)', marginTop: 2 }}>{hs[char.id] ? hs[char.id] : '—'}</div>
                </div>
              );
            })}
          </div>
        ...
        <div style={{ background: cardBg, border: '1px solid ' + cardBorder, borderRadius: 14, padding: '14px 12px' }}>
          ...
          <div style={{ display: 'flex' }}>
            {[
              { val: stats.games || 0,                  label: 'Voyages'    },
              { val: globalHi || '—',                   label: 'Best Score' },
              { val: stats.bestLevel ? 'Lv ' + stats.bestLevel : '—', label: 'Best Level' },
            ].map(function (item, i) {
              return (
        ...
        <div>
          ...
          {loading ? (
            <div style={{ padding: '20px 6px', textAlign: 'center', fontSize: 13, color: mutedColor }}>Loading captains…</div>
          ) : fullLb.length ? (
              ...
              {fullLb.map(function (entry, i) {
                ...
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 10px', borderRadius: 10, background: isYou ? (dark ? 'rgba(143,211,182,.17)' : 'var(--brand-tint)') : 'transparent' }}>
                    ...
                    <div style={{ width: 36, height: 36, borderRadius: '50%', background: dark ? 'rgba(255,255,255,.1)' : 'var(--bg-sunken)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
                      <RoutlFacePortrait charId={entry.char} size={36} />
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: mutedColor }}>{entryChar ? entryChar.name : entry.char}</div>
                    </div>
            ...
            <div style={{ padding: '22px 16px', textAlign: 'center', border: '1px dashed ' + cardBorder, borderRadius: 12 }}>
              ...
              <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--game-text)', marginBottom: 2 }}>No captains on the board yet</div>
              <div style={{ fontSize: 12, color: mutedColor }}>Set sail to claim the top spot!</div>
            </div>
          )}
        </div>
        ...
        <div style={{ background: dark ? 'rgba(255,255,255,.05)' : 'var(--bg-sunken)', borderRadius: 12, padding: '12px 14px' }}>
          ...
          {[
            '🧭 Tap or drag to step toward the treasure ✕',
            '🏴‍☠️ Pirates advance one step each move along their lines',
            '↩️ Double back to waste a turn and let a pirate pass',
            '⭐ Fewer moves scores more — find the optimal route',
...
window.RoutlLeaderboardScreen = RoutlLeaderboardScreen;
