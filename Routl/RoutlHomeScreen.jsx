// ROUTL — Home Screen with captain selector

function RoutlPreviewCanvas({ charId, size }) {
  ...
  React.useEffect(function () {
    ...
    function drawFrame(f) {
      ...
      ctx.save(); ctx.translate(Math.round(size * 0.43), size - 8);
      try { window.RoutlGame.drawCharacter(ctx, charId, f, false); } catch (e) { console.error(e); }
      ctx.restore();
    }
    drawFrame(30);
    function loop() { frameRef.current++; drawFrame(frameRef.current); rafRef.current = requestAnimationFrame(loop); }
    rafRef.current = requestAnimationFrame(loop);
    return function () { cancelAnimationFrame(rafRef.current); };
  }, [charId, size]);
  return <canvas ref={canvasRef} width={size} height={size} style={{ display: 'block', pointerEvents: 'none', width: size + 'px', height: size + 'px' }} />;
}

function RoutlHomeScreen({ theme, charId, setCharId, onPlay }) {
  ...
  var LOGO_COLORS  = ['#4FA3E0', '#3FA35A', '#F4B942', '#E2483D', '#8E5BD0'];
  var LOGO_LETTERS = ['R', 'O', 'U', 'T', 'L'];
      ...
      <div style={{ margin: 'auto', width: '100%', maxWidth: 420, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '48px 16px 20px', gap: 16 }}>
        <div style={{ display: 'flex', gap: 5 }}>
          {LOGO_LETTERS.map(function (letter, i) {
            return <div key={i} style={{ width: 38, height: 38, borderRadius: 9, background: LOGO_COLORS[i], display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 18, color: '#fff', boxShadow: '0 3px 0 rgba(0,0,0,.18)' }}>{letter}</div>;
          })}
        </div>

        <div style={{ width: '100%' }}>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: mutedColor, marginBottom: 8, marginTop: 20 }}>Choose Your Captain</div>
            ...
            {Game.CHARACTERS.map(function (char) {
              ...
              return (
                <button key={char.id} onClick={function () { setCharId(char.id); }} style={{
                  ...
                  borderRadius: 14, background: active ? CHAR_BG[char.id] : (dark ? 'rgba(255,255,255,.03)' : '#fff'),
                  cursor: 'pointer', padding: '6px 4px 10px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2,
                  boxShadow: active ? ('0 0 0 3px ' + cc + '28') : 'none', transition: 'all 150ms ease', WebkitTapHighlightColor: 'transparent',
                }}>
                  <RoutlPreviewCanvas charId={char.id} size={86} />
                  <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 14, color: active ? cc : 'var(--game-text)', lineHeight: 1 }}>{char.name}</div>
                  {hs[char.id] ? <div style={{ fontSize: 9, fontWeight: 700, color: mutedColor, marginTop: 1 }}>Best: {hs[char.id]}</div> : null}
                </button>
              );
            })}
          </div>
        ...
        <div style={{ width: '100%' }}>
          <Button variant="primary" size="lg" fullWidth onClick={onPlay}>Set Sail!</Button>
        </div>
...
window.RoutlHomeScreen = RoutlHomeScreen;
