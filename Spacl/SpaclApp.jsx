// SPACL — App root: full-viewport mobile shell, nav, screen routing

function SpaclBottomNav({ screen, onTab, theme }) {
      ...
      {tabs.map(function (tab) {
        ...
        return (
          <button key={tab.id} onClick={function () { onTab(tab.id); }} style={{
            ...
            transition: 'color 120ms ease', position: 'relative',
            ...
            {active && (
              <div style={{
                ...
                width: 20, height: 3, borderRadius: '3px 3px 0 0', background: 'var(--brand)',
              }}></div>
            )}
          </button>
        );
...
function SpaclApp() {
  ...
  var [theme,   setTheme]   = React.useState(function () { return localStorage.getItem('spacl-theme') || 'clean-light'; });
  var [charId,  setCharId]  = React.useState(function () { return localStorage.getItem('spacl-char')  || 'B'; });
  ...
  var saveTheme = function (t) { setTheme(t); localStorage.setItem('spacl-theme', t); };
  var saveChar  = function (c) { setCharId(c); localStorage.setItem('spacl-char', c); };
  ...
  return (
    <div className="app-root" data-theme={theme}>
      <div className="phone-shell">
        {screen === 'home' && React.createElement(window.SpaclHomeScreen, {
          theme, charId, setCharId: saveChar, onPlay: startGame,
        })}
        {screen === 'game' && React.createElement(window.SpaclGameScreen, {
          key: gameKey, theme, charId,
          onBack:  function () { setScreen('home'); },
          onBoard: function () { setScreen('leaderboard'); },
        })}
        {screen === 'leaderboard' && React.createElement(window.SpaclLeaderboardScreen, { theme, charId })}
        {screen === 'settings' && React.createElement(window.SpaclSettingsScreen, { theme, setTheme: saveTheme })}
        {showNav && React.createElement(SpaclBottomNav, { screen, onTab: handleTab, theme })}
