// SPACL — Home Screen with animated pilot selector

// Mini canvas preview — draws a pet (side view) in a card
function SpaclPreviewCanvas({ charId, size }) {
  ...
  React.useEffect(function () {
    ...
    function drawFrame(f) {
      ...
      try { window.SpaclGame.drawCharacter(ctx, charId, f, false); } catch (e) { console.error(e); }
      ctx.restore();
    }
    drawFrame(30);
...
function SpaclHomeScreen({ theme, charId, setCharId, onPlay }) {
  ...
  var LOGO_COLORS  = ['#4FA3E0', '#8FD3B6', '#F4B942', '#E8755C', '#C7B6E8'];
  var LOGO_LETTERS = ['S', 'P', 'A', 'C', 'L'];
  ...
  return (
    <div style={{
      ...
      background: 'var(--game-bg)', color: 'var(--game-text)', overflowY: 'auto', minHeight: 0,
    }}>
      <button onClick={function () { window.location.href = '../index.html'; }} aria-label="Back to game selection"
        style={{
          position: 'absolute', top: 6, left: 6, zIndex: 5, width: 38, height: 38, borderRadius: '50%',
          border: 'none', background: 'transparent', cursor: 'pointer', color: 'var(--game-text)',
      ...
      <div style={{ margin: 'auto', width: '100%', maxWidth: 420, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '48px 16px 20px', gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 5 }}>
          {LOGO_LETTERS.map(function (letter, i) {
            return (
              <div key={i} style={{
                width: 38, height: 38, borderRadius: 9, background: LOGO_COLORS[i],
            ...
            {Game.CHARACTERS.map(function (char) {
              ...
              return (
                <button key={char.id} onClick={function () { setCharId(char.id); }} style={{
                  ...
                  <SpaclPreviewCanvas charId={char.id} size={86} />
                  ...
                  {hs[char.id] ? (
                    <div style={{ fontSize: 9, fontWeight: 700, color: mutedColor, marginTop: 1 }}>Best: {hs[char.id]}</div>
                  ) : null}
                </button>
              );
            })}
        ...
        {/* Play button */}
        <div style={{ width: '100%' }}>
          <Button variant="primary" size="lg" fullWidth onClick={onPlay}>Launch!</Button>
        </div>
...
window.SpaclHomeScreen = SpaclHomeScreen;
