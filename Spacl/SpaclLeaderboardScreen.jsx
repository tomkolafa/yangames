// SPACL — Leaderboard Screen

// Front-facing pet portrait (reuses SpaclGame.drawFace — same art as the ship cockpit)
function SpaclFacePortrait({ charId, size }) {
  ...
  React.useEffect(function () {
    ...
    try { window.SpaclGame.drawFace(ctx, charId, s); } catch (e) { console.error(e); }
    ctx.restore();
  }, [charId, s]);
  return <canvas ref={canvasRef} style={{ display: 'block', width: s + 'px', height: s + 'px', margin: '0 auto' }} />;
}

function SpaclLeaderboardScreen({ theme, charId }) {
  ...
  React.useEffect(function () {
    if (window.YanLeaderboard && window.YanLeaderboard.ENABLED) {
      window.YanLeaderboard.fetchTop('spacl', { limit: 25 }).then(function (rows) { setOnline(rows || []); });
    } else { setOnline([]); }
  }, []);
  ...
  if (online && online.length) {
    ...
    Game.CHARACTERS.forEach(function (c) {
      if (hs[c.id]) fullLb.push({ name: myName || 'You', emoji: CHAR_EMOJI[c.id], char: c.id, score: hs[c.id], isYou: true });
    });
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--game-bg)', color: 'var(--game-text)', minHeight: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px 28px', display: 'flex', flexDirection: 'column', gap: 20, minHeight: 0 }}>
        ...
        {/* Per-pilot bests */}
        <div>
          <SectionLabel>Your Best Per Pilot</SectionLabel>
            ...
            {Game.CHARACTERS.map(function (char) {
              ...
              return (
                <div key={char.id} style={{
                  ...
                  border: '1px solid ' + (active ? cc : cardBorder), borderRadius: 12, padding: '12px 10px', textAlign: 'center',
                }}>
                  <div style={{ marginBottom: 4 }}><SpaclFacePortrait charId={char.id} size={44} /></div>
        ...
        <div>
          ...
          {loading ? (
            <div style={{ padding: '20px 6px', textAlign: 'center', fontSize: 13, color: mutedColor }}>Loading pilots…</div>
          ) : fullLb.length ? (
                ...
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 10px', borderRadius: 10, background: isYou ? (dark ? 'rgba(143,211,182,.17)' : 'var(--brand-tint)') : 'transparent' }}>
                    ...
                    <div style={{ width: 36, height: 36, borderRadius: '50%', background: dark ? 'rgba(255,255,255,.1)' : 'var(--bg-sunken)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
                      <SpaclFacePortrait charId={entry.char} size={36} />
                    </div>
            ...
            <div style={{ padding: '22px 16px', textAlign: 'center', border: '1px dashed ' + cardBorder, borderRadius: 12 }}>
              ...
              <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--game-text)', marginBottom: 2 }}>No pilots on the board yet</div>
              <div style={{ fontSize: 12, color: mutedColor }}>Fly a mission to claim the top spot!</div>
            </div>
          )}
        </div>
        ...
        <div style={{ background: dark ? 'rgba(255,255,255,.05)' : 'var(--bg-sunken)', borderRadius: 12, padding: '12px 14px' }}>
          ...
          {[
            '🕹️ Drag to fly — your blaster auto-fires',
            '✦ Tap the missile button for a homing volley',
            '🛡️ Grab power-ups: missiles, laser, shield, extra life',
            '👾 Clear every wave, then beat the sector boss',
          ].map(function (tip) {
            return <div key={tip} style={{ fontSize: 12, fontWeight: 500, color: bodyColor, lineHeight: 1.6 }}>{tip}</div>;
          })}
        </div>
      </div>
    </div>
...
window.SpaclLeaderboardScreen = SpaclLeaderboardScreen;
