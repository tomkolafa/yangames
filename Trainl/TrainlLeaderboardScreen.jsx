// TRAINL — Leaderboard Screen

function TrainlFacePortrait({ charId, size }) {
  ...
  React.useEffect(function () {
    ...
    try { window.TrainlGame.drawFace(ctx, charId, s); } catch (e) { console.error(e); }
    ctx.restore();
  }, [charId, s]);
...
function TrainlLeaderboardScreen({ theme, charId }) {
  ...
  React.useEffect(function () {
    if (window.YanLeaderboard && window.YanLeaderboard.ENABLED) {
      window.YanLeaderboard.fetchTop('trainl', { limit: 25 }).then(function (rows) {
        setOnline(rows || []);
      });
    } else {
      setOnline([]);
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--game-bg)', color: 'var(--game-text)', minHeight: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px 28px', display: 'flex', flexDirection: 'column', gap: 20, minHeight: 0 }}>
              ...
              return (
                <div key={char.id} style={{
                  ...
                  <div style={{ marginBottom: 4 }}>
                    <TrainlFacePortrait charId={char.id} size={44} />
                  </div>
          ...
          {loading ? (
              ...
              {fullLb.map(function (entry, i) {
                ...
                return (
                  <div key={i} style={{
                    ...
                    background: isYou ? (dark ? 'rgba(79,163,224,.2)' : 'var(--brand-tint)') : 'transparent',
                  }}>
                    ...
                    <div style={{
                      ...
                      <TrainlFacePortrait charId={entry.char} size={36} />
                    </div>
            ...
            <div style={{
              ...
              <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--game-text)', marginBottom: 2 }}>
                No dispatchers on the board yet
              </div>
              <div style={{ fontSize: 12, color: mutedColor }}>
                Run the rails to claim the top spot!
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{
          ...
          {[
            '🚆 Trains leave the depot in different colours',
            '🔀 Tap a switch to flip which way it sends them',
            '🎯 Route each train to its matching station',
            '💥 Wrong station or a crash costs a life',
          ].map(function (tip) {
            return (
...
window.TrainlLeaderboardScreen = TrainlLeaderboardScreen;
