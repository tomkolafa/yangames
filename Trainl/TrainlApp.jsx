// TRAINL — App root: full-viewport mobile shell, nav, screen routing

function TrainlBottomNav({ screen, onTab, theme }) {
...
function TrainlApp() {
  ...
  var [theme,   setTheme]   = React.useState(function () { return localStorage.getItem('trainl-theme') || 'clean-light'; });
  var [charId,  setCharId]  = React.useState(function () { return localStorage.getItem('trainl-char')  || 'B'; });
  ...
  var saveTheme = function (t) { setTheme(t); localStorage.setItem('trainl-theme', t); };
  var saveChar  = function (c) { setCharId(c); localStorage.setItem('trainl-char', c); };
  ...
  return (
    <div className="app-root" data-theme={theme}>
      <div className="phone-shell">
        {screen === 'home' && React.createElement(window.TrainlHomeScreen, {
          theme, charId, setCharId: saveChar, onPlay: startGame,
        })}
        {screen === 'game' && React.createElement(window.TrainlGameScreen, {
          key: gameKey,
          theme, charId,
          onBack:  function () { setScreen('home'); },
          onBoard: function () { setScreen('leaderboard'); },
        })}
        {screen === 'leaderboard' && React.createElement(window.TrainlLeaderboardScreen, {
          theme, charId,
        })}
        {screen === 'settings' && React.createElement(window.TrainlSettingsScreen, {
          theme, setTheme: saveTheme,
        })}
        {showNav && React.createElement(TrainlBottomNav, { screen, onTab: handleTab, theme })}
