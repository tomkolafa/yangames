// TRAINL — Home Screen with animated pet selector

function TrainlPreviewCanvas({ charId, size }) {
  ...
  React.useEffect(function () {
    ...
    function drawFrame(f) {
      ...
      try { window.TrainlGame.drawCharacter(ctx, charId, f, false); } catch (e) { console.error(e); }
      ctx.restore();
    }
...
function TrainlHomeScreen({ theme, charId, setCharId, onPlay }) {
  ...
  var LOGO_COLORS  = ['#E8755C', '#4FA3E0', '#56C98A', '#F4B942', '#9B6BD6', '#79D0E0'];
  var LOGO_LETTERS = ['T', 'R', 'A', 'I', 'N', 'L'];
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ fontSize: 12, fontWeight: 600, color: mutedColor }}>Flip the switches, deliver every train</div>
      </div>
          ...
          {Game.CHARACTERS.map(function (char) {
            ...
            return (
              <button key={char.id} onClick={function () { setCharId(char.id); }} style={{
                ...
                <TrainlPreviewCanvas charId={char.id} size={86} />
                <div style={{
...
window.TrainlHomeScreen = TrainlHomeScreen;
