// BREWL — Leaderboard Screen

function BrewlFacePortrait({ charId, size }) {
  ...
  React.useEffect(function () {
    ...
    try { window.BrewlGame.drawFace(ctx, charId, s); } catch (e) { console.error(e); }
    ctx.restore();
  }, [charId, s]);
...
function BrewlLeaderboardScreen({ theme, charId }) {
  ...
  React.useEffect(function () {
    if (window.YanLeaderboard && window.YanLeaderboard.ENABLED) {
      window.YanLeaderboard.fetchTop('brewl', { limit: 25 }).then(function (rows) {
        setOnline(rows || []);
      });
    } else {
      setOnline([]);
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--game-bg)', color: 'var(--game-text)', minHeight: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px 28px', display: 'flex', flexDirection: 'column', gap: 20, minHeight: 0 }}>
              ...
              return (
                <div key={char.id} style={{
                  ...
                  <div style={{ marginBottom: 4 }}>
                    <BrewlFacePortrait charId={char.id} size={44} />
                  </div>
          ...
          {loading ? (
              ...
              {fullLb.map(function (entry, i) {
                ...
                return (
                  <div key={i} style={{
                    ...
                    background: isYou ? (dark ? 'rgba(155,107,58,.2)' : 'var(--brand-tint)') : 'transparent',
                  }}>
                    ...
                    <div style={{
                      ...
                      <BrewlFacePortrait charId={entry.char} size={36} />
                    </div>
            ...
            <div style={{
              ...
              <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--game-text)', marginBottom: 2 }}>
                No baristas on the board yet
              </div>
              <div style={{ fontSize: 12, color: mutedColor }}>
                Work a shift to claim the top spot!
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{
          ...
          {[
            '☕ Cups fill on their own — watch them all',
            '🎯 Tap a cup when it reaches the green line',
            '🌊 Let one overflow and you lose a life',
            '🔔 Tap the bell to clear an interruption',
          ].map(function (tip) {
            return (
...
window.BrewlLeaderboardScreen = BrewlLeaderboardScreen;
