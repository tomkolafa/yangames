// BREWL — App root: full-viewport mobile shell, nav, screen routing

function BrewlBottomNav({ screen, onTab, theme }) {
...
function BrewlApp() {
  ...
  var [theme,   setTheme]   = React.useState(function () { return localStorage.getItem('brewl-theme') || 'clean-light'; });
  var [charId,  setCharId]  = React.useState(function () { return localStorage.getItem('brewl-char')  || 'B'; });
  ...
  var saveTheme = function (t) { setTheme(t); localStorage.setItem('brewl-theme', t); };
  var saveChar  = function (c) { setCharId(c); localStorage.setItem('brewl-char', c); };
  ...
  return (
    <div className="app-root" data-theme={theme}>
      <div className="phone-shell">
        {screen === 'home' && React.createElement(window.BrewlHomeScreen, {
          theme, charId, setCharId: saveChar, onPlay: startGame,
        })}
        {screen === 'game' && React.createElement(window.BrewlGameScreen, {
          key: gameKey,
          theme, charId,
          onBack:  function () { setScreen('home'); },
          onBoard: function () { setScreen('leaderboard'); },
        })}
        {screen === 'leaderboard' && React.createElement(window.BrewlLeaderboardScreen, {
          theme, charId,
        })}
        {screen === 'settings' && React.createElement(window.BrewlSettingsScreen, {
          theme, setTheme: saveTheme,
        })}
        {showNav && React.createElement(BrewlBottomNav, { screen, onTab: handleTab, theme })}
