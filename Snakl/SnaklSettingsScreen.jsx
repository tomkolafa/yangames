// SNAKL — Settings Screen (shared chrome + Snakl-specific walls toggle)
function SnaklSettingsScreen({ theme, setTheme }) {
  var dark = theme === 'clean-dark' || theme === 'funky';

  var [sound, setSound] = React.useState(function() { return localStorage.getItem('snakl-sound') !== '0'; });
  // Default OFF → walls pass through (wrap). '1' = solid walls (hitting a wall ends the game).
  var [solidWalls, setSolidWalls] = React.useState(function() { return localStorage.getItem('snakl-walls') === '1'; });

  var mutedColor = dark ? 'rgba(255,255,255,.45)' : 'var(--text-muted)';
  var divColor   = dark ? 'rgba(255,255,255,.08)' : 'var(--hairline)';

  var saveSound = function(v) { setSound(v); localStorage.setItem('snakl-sound', v ? '1' : '0'); };
  var saveWalls = function(v) { setSolidWalls(v); localStorage.setItem('snakl-walls', v ? '1' : '0'); };
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--game-bg)', color: 'var(--game-text)', minHeight: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 18px 32px', minHeight: 0 }}>
        ...
        <Row label="Solid Walls"
          desc="Off: pass through walls and wrap to the other side. On: hitting a wall ends the game."
          control={React.createElement(Toggle, { value: solidWalls, onChange: saveWalls })} />
        <Row label="Sound Effects"
          control={React.createElement(Toggle, { value: sound, onChange: saveSound })} />

        {/* About */}
        <SectionLabel>About</SectionLabel>
        <Row label="Snakl" desc="Classic snake — guide your pet, eat treats, grow long" control={
...
window.SnaklSettingsScreen = SnaklSettingsScreen;
