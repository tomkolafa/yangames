// SNAKL — Leaderboard Screen

// Front-facing pet portrait (reuses SnaklGame.drawFace — same art as the head)
function SnaklFacePortrait({ charId, size }) {
  ...
  React.useEffect(function () {
    ...
    try { window.SnaklGame.drawFace(ctx, charId, s); } catch (e) { console.error(e); }
    ctx.restore();
  }, [charId, s]);
...
function SnaklLeaderboardScreen({ theme, charId }) {
  ...
  React.useEffect(function () {
    if (window.YanLeaderboard && window.YanLeaderboard.ENABLED) {
      window.YanLeaderboard.fetchTop('snakl', { limit: 25 }).then(function (rows) {
        setOnline(rows || []);
      });
    } else {
      setOnline([]);
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--game-bg)', color: 'var(--game-text)', minHeight: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px 28px', display: 'flex', flexDirection: 'column', gap: 20, minHeight: 0 }}>
        ...
        {/* ── Per-character bests ── */}
        <div>
          <SectionLabel>Your Best Per Pet</SectionLabel>
              ...
              return (
                <div key={char.id} style={{
                  ...
                  <div style={{ marginBottom: 4 }}>
                    <SnaklFacePortrait charId={char.id} size={44} />
                  </div>
        ...
        <div style={{ background: cardBg, border: '1px solid ' + cardBorder, borderRadius: 14, padding: '14px 12px' }}>
          ...
          <div style={{ display: 'flex' }}>
            {[
              ...
              { val: globalHi || '—',     label: 'Best Length' },
                ...
                return (
                  <div key={i} style={{
                    ...
                    <div style={{
                      ...
                      <SnaklFacePortrait charId={entry.char} size={36} />
                    </div>
        ...
        <div style={{
          ...
          {[
            '🍽️ Eat food to grow — each bite is a point',
            '🍦🍕🍗🍣 The treat rotates with every bite',
            '🚫 Don\'t crash into your own tail',
            '🧱 Walls wrap by default — flip on Solid Walls in Settings',
          ].map(function (tip) {
            return (
...
window.SnaklLeaderboardScreen = SnaklLeaderboardScreen;
