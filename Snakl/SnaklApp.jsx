// SNAKL — App root: full-viewport mobile shell, nav, screen routing

function SnaklBottomNav({ screen, onTab, theme }) {
...
function SnaklApp() {
  ...
  var [theme,   setTheme]   = React.useState(function () { return localStorage.getItem('snakl-theme') || 'clean-light'; });
  var [charId,  setCharId]  = React.useState(function () { return localStorage.getItem('snakl-char')  || 'B'; });
  ...
  var saveTheme = function (t) { setTheme(t); localStorage.setItem('snakl-theme', t); };
  var saveChar  = function (c) { setCharId(c); localStorage.setItem('snakl-char', c); };
  ...
  return (
    <div className="app-root" data-theme={theme}>
      <div className="phone-shell">
        {screen === 'home' && React.createElement(window.SnaklHomeScreen, {
          theme, charId, setCharId: saveChar, onPlay: startGame,
        })}
        {screen === 'game' && React.createElement(window.SnaklGameScreen, {
          key: gameKey,
          theme, charId,
          onBack:  function () { setScreen('home'); },
          onBoard: function () { setScreen('leaderboard'); },
        })}
        {screen === 'leaderboard' && React.createElement(window.SnaklLeaderboardScreen, {
          theme, charId,
        })}
        {screen === 'settings' && React.createElement(window.SnaklSettingsScreen, {
          theme, setTheme: saveTheme,
        })}
        {showNav && React.createElement(SnaklBottomNav, { screen, onTab: handleTab, theme })}
