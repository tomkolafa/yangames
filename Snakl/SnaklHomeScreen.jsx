// SNAKL — Home Screen with animated pet selector

// Mini canvas preview — draws a pet (side view) running in a card
function SnaklPreviewCanvas({ charId, size }) {
  ...
  React.useEffect(function () {
    ...
    function drawFrame(f) {
      ...
      try { window.SnaklGame.drawCharacter(ctx, charId, f, false); } catch (e) { console.error(e); }
      ctx.restore();
    }
...
function SnaklHomeScreen({ theme, charId, setCharId, onPlay }) {
  ...
  var LOGO_COLORS  = ['#8FD3B6', '#F4B942', '#E8755C', '#C7B6E8', '#A8D8EA'];
  var LOGO_LETTERS = ['S', 'N', 'A', 'K', 'L'];
  ...
  return (
    <div style={{
          ...
          {Game.CHARACTERS.map(function (char) {
            ...
            return (
              <button key={char.id} onClick={function () { setCharId(char.id); }} style={{
                ...
                <SnaklPreviewCanvas charId={char.id} size={86} />
                <div style={{
      ...
      <div style={{
        ...
        {[
          { val: globalHi > 0 ? String(globalHi) : '—', label: 'Best Length' },
...
window.SnaklHomeScreen = SnaklHomeScreen;
